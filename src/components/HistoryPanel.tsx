...
const HistoryItem: React.FC<{ ticket: Ticket }> = ({ ticket }) => {
  const getStrategyColor = (strategy: string) => {
    switch (strategy) {
      case 'The Iron Bank': return 'bg-blue-500/20 text-blue-400';
      case 'The Bookie Basher': return 'bg-amber-500/20 text-amber-400';
      case 'The High-Yield Assassin': return 'bg-red-500/20 text-red-400';
    ...
    <div className="bg-slate-900/30 border border-slate-700 rounded-lg p-4 hover:border-slate-600 transition-colors">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center space-x-3">
          <span className={`text-xs font-bold px-2 py-1 rounded-full ${getStrategyColor(ticket.strategy)}`}>
            {ticket.strategy}
          </span>
